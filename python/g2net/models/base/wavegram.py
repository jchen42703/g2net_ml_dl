...
from .resnet1d import BasicBlock, MyConv1dPadSame
...
class WaveNetSpectrogram(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(self.out_chans):
            tmp_blocks = [
                wave_block(wave_layers[i], in_channels, base_filters[0],
                           kernel_size, downsample)
            ]
            if len(base_filters) > 1:
                for j in range(len(base_filters) - 1):
                    tmp_blocks = tmp_blocks + [
                        wave_block(wave_layers[i], base_filters[j],
                                   base_filters[j + 1], kernel_size, downsample)
                    ]
