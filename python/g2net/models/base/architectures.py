...
from .pooling import AdaptiveConcatPool2d, GeM
from .nn_modules import TripletAttention, ManifoldMixup, PositionalEncoding, \
                        Flatten, mixup, cutmix
...
class SpectroCNN(nn.Module):
    ...
    def __init__(self,
        ...
        self.is_cnnspec = self.spectrogram.__class__.__name__ in [
            'WaveNetSpectrogram', 'CNNSpectrogram', 'MultiSpectrogram',
            'ResNetSpectrogram'
        ]
        ...
        if custom_classifier != 'none' or custom_attention != 'none':
            ...
            if custom_preprocess == 'positional-add':
                preprocess = PositionalEncoding(feature_dim=resize_img,
                                                mode='add')
            ...
            #     global_pool = nn.Sequential(AdaptiveGeM((1, None), p=3),
            #                                 PositionalEncoding(),
            #                                 nn.AdaptiveAvgPool2d((1, 1)))
                ...
                raise ValueError(
                    f'Unsupported classifier type: {custom_classifier}')

            if model_type[
                    -11:] == 'Transformer' or model_type == 'XCiT':  # transformer models
                global_pool = nn.Identity()

            self.cnn = nn.Sequential(preprocess, self.cnn, attention,
                                     global_pool, Flatten(),
    ...
    def forward(self,
                s,
                lam=None,
                idx=None):  # s: (batch size, wave channel, length of wave)
        bs, ch, w = s.shape
        if self.is_cnnspec:  # Use CNN as spectrogram
            ...
            with torch.cuda.amp.autocast(
                    enabled=False):  # MelSpectrogram causes NaN with AMP
                spec = self.spectrogram(
                    s)  # spec: (batch size * wave channel, freq, time)
            _, f, t = spec.shape
