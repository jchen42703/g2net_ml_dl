...
class MiniRocketFeatures(nn.Module):
    ...
    def __init__(self,
        ...
        # Convolution
        indices = torch.combinations(torch.arange(self.kernel_size),
                                     3).unsqueeze(1)
        kernels = (-torch.ones(self.num_kernels, 1, self.kernel_size)).scatter_(
            2, indices, 2)
        self.kernels = nn.Parameter(kernels.repeat(c_in, 1, 1),
                                    requires_grad=False)
    ...
    def forward(self, x):
        _features = []
        for i, (dilation,
                padding) in enumerate(zip(self.dilations, self.padding)):
            _padding1 = i % 2
            ...
            if not self.prefit or self.fitting:
                ...
                bias_this_dilation = self._get_bias(C,
            ...
            # Features
            _features.append(
                self._get_PPVs(C[:, _padding1::2],
                               bias_this_dilation[_padding1::2]))
            _features.append(
                self._get_PPVs(C[:, 1 - _padding1::2, padding:-padding],
    ...
    def _set_dilations(self, input_length):
        ...
        true_max_dilations_per_kernel = min(num_features_per_kernel,
    ...
    def _set_channel_combinations(self, num_channels):
        ...
        np.random.seed(self.random_state)
        num_channels_per_combination = (2**np.random.uniform(
            0, max_exponent_channels, num_combinations)).astype(np.int32)
        ...
        for i in range(num_combinations):
            channel_combinations[:,
                                 np.random.
                                 choice(num_channels,
                                        num_channels_per_combination[i], False),
                                 i] = 1
        channel_combinations = torch.split(channel_combinations,
                                           self.num_kernels,
                                           2)  # split by dilation
        for i, channel_combination in enumerate(channel_combinations):
            self.register_buffer(f'channel_combinations_{i}',
...
def get_minirocket_features(o,
    ...
    device = torch.device(
        torch.cuda.current_device()) if use else torch.device('cpu')
...
class MiniRocketHead(nn.Sequential):
    ...
    def __init__(self, c_in, c_out, seq_len=1, bn=True, fc_dropout=0.):
        ...
        super().__init__(
            OrderedDict([('backbone', nn.Sequential()), ('head', head)]))
